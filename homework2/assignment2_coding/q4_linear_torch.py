...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        self.input_size = self.img_height * self.img_width * self.n_channels * self.state_history
        ...
        ################ YOUR CODE HERE (3-4 lines) ##################
         # Q network
        self.q_network = nn.Sequential(nn.Linear(self.input_size, 128), nn.ReLU(), nn.Linear(128, self.num_actions))

        # Target network
        self.target_network = nn.Sequential(nn.Linear(self.input_size, 128), nn.ReLU(), nn.Linear(128, self.num_actions))
        self.target_network.load_state_dict(self.q_network.state_dict())
    ...
    def calc_loss(
        ...
        chosen_action_q_values = torch.sum(q_values * F.one_hot(actions, num_actions), dim=1)

    # Compute the expected Q values from the target network
        expected_q_values = torch.max(target_q_values, dim=1).values
        target_q_values = rewards + gamma * expected_q_values * (1 - done_mask)

    # Compute the MSE loss
        loss = F.mse_loss(chosen_action_q_values, target_q_values)
